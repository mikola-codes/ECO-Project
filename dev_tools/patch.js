...
`;
content = content.replace(/(<!-- Confirm Modal -->)/, notifContent + '\n    $1');
...
const injectedJs = `
        ...
        function saveNotifSettings() {
            ...
            fetch('api/notification_settings.php', { method: 'POST', body: fd }).then(r => r.json()).then(d => {
                showMsg(document.getElementById('nsMsg'), d.success ? '✅ Saved' : '❌ ' + d.message, d.success ? 'ok' : 'err');
            }).catch(e => showMsg(document.getElementById('nsMsg'), '❌ ' + e, 'err'));
        }
        ...
        // Add to switchTab logic implicitly or add event listeners
        ...
        if(startEnrollBtn) {
            startEnrollBtn.addEventListener('click', function() {
                ...
                // Construct final formData (bypassing the 10-finger scan logic since it was deleted by a previous agent and we only need to pass the new email/phone)
                var finalFd = new FormData();
                finalFd.append('nickname', nn);
                finalFd.append('email', document.getElementById('enrollEmail').value.trim());
                finalFd.append('phone_number', document.getElementById('enrollPhone').value.trim());
                ...
                fetch('api/enroll.php', { method: 'POST', body: finalFd })
                    ...
                    .then(data => {
                        ...
                        startEnrollBtn.textContent = 'Start 10-Finger Enrollment &rarr;';
                        if (data.success) {
...
content = content.replace(/(function loadHistory\(\) {[\s\S]*?})/, '$1\n' + injectedJs);
